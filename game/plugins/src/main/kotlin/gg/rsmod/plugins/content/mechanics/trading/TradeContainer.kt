...
class TradeContainer(private val player: Player) {
    ...
    private var onItemOffered : (Player.(Item) -> Unit)? = null

    fun offer(slot: Int, amount: Int) {
        val item = player.inventory.get(slot) ?: return
        onItemOffered?.invoke(player, item)
    }

    fun refresh() {
        player.sendItemContainer(key = 93, container = inventory)
    }

    fun bindItemOffered(onItemOffered: Player.(Item) -> Unit) {
