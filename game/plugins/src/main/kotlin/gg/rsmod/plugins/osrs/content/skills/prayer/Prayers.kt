...
object Prayers {
    ...
    val PRAYER_DRAIN_COUNTER = AttributeKey<Int>()
    ...
    const val ACTIVE_PRAYERS_VARP = 83
    const val SELECTED_QUICK_PRAYERS_VARP = 84

    const val INF_PRAY_VARBIT = 5314
    const val QUICK_PRAYERS_ACTIVE_VARBIT = 4103
    ...
    suspend fun checkRequirements(it: Plugin, prayer: Prayer): Boolean {
        val p = it.player()
    ...
    fun calculateDrainRate(p: Player): Int = Prayer.values.filter { isActive(p, it) }.sumBy { it.drainEffect }
}