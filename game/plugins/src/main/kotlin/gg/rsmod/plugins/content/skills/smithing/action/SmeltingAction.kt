...
class SmeltingAction(private val defs: DefinitionSet) {
    ...
    suspend fun smelt(task: QueueTask, bar: Bar, amount: Int) {
        ...
        repeat(maxCount) {
            ...
            if (bar.primaryOre == Items.COPPER_ORE || bar.primaryOre == Items.TIN_ORE) {
                player.filterableMessage("You smelt the copper and tin together in the furnace.")
            } else
            if (bar.primaryOre == Items.SILVER_ORE) {
                player.filterableMessage("You place a lump of silver in the furnace.")
            } else
            player.filterableMessage("you smelt the ${bar.prefix} in the furnace.")
