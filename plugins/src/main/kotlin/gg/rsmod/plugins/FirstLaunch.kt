package gg.rsmod.plugins

import gg.rsmod.game.plugin.Plugin
import gg.rsmod.game.plugin.PluginRepository
import gg.rsmod.game.plugin.ScanPlugins

/**
 * @author Tom <rspsmods@gmail.com>
 */
object FirstLaunch {

    @JvmStatic
    @ScanPlugins
    fun register(r: PluginRepository) {
        r.bindLogin {
            if (it.player().world.gameContext.initialLaunch) {
                it.player().world.gameContext.initialLaunch = false
                it.suspendable { dialog(it) }
            }
        }
    }

    private suspend fun dialog(it: Plugin) {
        val api = it.player().world.server.getApiName()
        val site = it.player().world.server.getApiSite()

        it.player().graphic(id = 1388, height = 124)
        it.doubleItemDialog("<u=801700><col=801700>Welcome to $api</col></u><br><br>Welcome to your new server, ${it.player().username}!", item1 = 11863, item2 = 11847)
        it.player().graphic(id = 1388, height = 124)
        it.itemDialog("Be sure to go over the <col=801700>README</col> file on the project root directory for basic information on the programming aspect of <col=801700>$api</col>.", item = 757)
        it.itemDialog("You can visit the official website at <col=801700>$site</col> to make suggestions, purchase & sell plugins, or report bugs.", item = 13652)
    }
}